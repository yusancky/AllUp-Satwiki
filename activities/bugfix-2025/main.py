...
def show_score(rank, score):
    if rank == 1:
        return f"""<font color="#D6E" style="font-family: formula1-black";>{score}</font>"""
    if rank <= 3:
        return f"""<font color="#4E4" style="font-family: formula1-black";>{score}</font>"""
    if rank <= 5:
        return f"""<font style="font-family: formula1-black";>{score}</font>"""
    if rank <= 7:
        return f"""<font style="font-family: TitilliumWeb-Bold";><b>{score}</b></font>"""
...
if __name__ == "__main__":
    ...
    leaderboard += f'|-\n| colspan="3" style="text-align:left;" | 排行榜由<font style="font-family: formula1-black";>[https://github.com/yusancky/AllUp-Satwiki AllUp]</font>每小时自动获取数据并更新。<small>上次更新：<code><font style="font-family: formula1-black";>[https://sat.huijiwiki.com/w/index.php?title=博客:天热了，让你站破产吧&oldid={revid} @{revid}]</font></code></small>\n|}}'
